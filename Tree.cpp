...
bool node_dot_print( const node_t* node, FILE* dot, bool s1mple_mode = false )
    ...
    fprintf( dot, "NODE%p[ shape = record, label = \" NODE: %p | ELEM: %s | Allocated: %d | { LEFT SON: %p | RIGHT SON: %p | "
                  "PARENT: %p } \" ];\n", node, node, node->val.ptr, node->val.allocated, node->left, node->right, node->up );
...
int main()
    ...
    tree_dot_dump( root, "tree1", true, true );
    tree_delete( root );
    return 0;
    }